...
if(!isset($_SESSION['mail']) OR empty($_SESSION['mail']))
{header('Location:blog/se-connecter?url=panier');}

$ids = array_keys($_SESSION['panier']);


if(isset($ids) AND !empty($ids)){ //condition panier existant
$numb=0;
$req2 = $bdd->query('SELECT * FROM produits WHERE id IN ('.implode(',',$ids).')');
while($donnees2=$req2->fetch()){
	$numb++;
		if(isset($donnees2['obs_url']) AND $donnees2['obs_url']=="libreP"){ header("Location:panier-libre.php");}
		if($numb>30)	{header("Location:trop-de-produits-panier");}
}
...
@-moz-document url-prefix(){
  ...
  top: -20px;
	...
	<title>Page Panier vincent-dev-web</title>
	<meta name="description" content="Vous pouvez ajouter vos produits qui sont ensuite visible sur cette page, vincent-dev-web."/>
</head>
<body>
<?php require 'header.php'; ?>
<div class="panier_page">
<h2 id="h2_pan">Panier</h2>
<div class="br_ff"><br><br><br><br></div>
	<?php if(isset($_SESSION['message']) AND !empty($_SESSION['message'])){echo '<h2 style="margin:auto;background:gray;text-align:center;color:red;width:330px">'.$_SESSION['message'].'</h2><br><br>'; $_SESSION['message']="";} ?>
	...
	<!--form-->
		...
		$livraison2=htmlspecialchars($livr21['livraison']);
			...
			$nbrart=$panier->count();
			$livraison5=htmlspecialchars($livr33['livraison']);
			$livraison4=$livraison5 * $_SESSION['panier'][$produit->id];



		$livraison2+=htmlspecialchars($livraison4);
		$livraison3=number_format($livraison2,2,'.',' ');
...
<div class="contenair_prod">	
			<form class="form1" method="post" action="panier.php">
				<a href="#">				
<?php if(isset($produit->img_type) AND $produit->img_type=="image"){ ?>		
	<img class="descr_prod1_img" src="publicimgs/<?php if($ifpromo=="oui"){echo 'promo-'.$produit->cle_image;}else{echo $produit->cle_image;} ?>"/>
<?php }
 if(isset($produit->img_type) AND $produit->img_type=="youtube"){ ?><iframe class="descr_prod1_img" <?php echo $produit->cle_image;?> title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
 	<?php
	} 
	if(isset($produit->img_type) AND $produit->img_type=="video"){ ?>
	<video class="descr_prod1_img" src="publicimgs/<?php echo $produit->cle_image;$req4=$bdd->prepare('SELECT * FROM video WHERE nom=?'); $req4->execute(array($produit->cle_image)); $donnees4=$req4->fetch();?>" controls poster="publicimgs/<?php echo $donnees4['image0'];?>" class="quatre" style="text-align:center"></video>
<?php }?></a><br>
				<p class="p_panier1" style="text-align:center;margin:auto"><?= $produit->titre; ?></p>
				<p class="p_prix">Prix : <?= number_format($produit->prix,2,',',' ');if(isset($donnees210['ok']) AND $donnees210['ok']!='oui'){echo ' €';} else {echo ' $';} ?></p>
				<p class="p_panier"><input type="button" id="m<?= $k;?>" value="--" style="width:22px:height:22px;border:1px solid black" /><input class="a<?= $k;?> inp_99_a" style="width:50px;font-size:14px" type="text" class="inp_panier" name="panier[quantite][<?= $produit->id; ?>]" value="<?= $_SESSION['panier'][$produit->id]; ?>"/><input type="button" id="p<?= $k;?>" value="+" style="width:22px;border:1px solid black"/>&nbsp;&nbsp;&nbsp;&nbsp;<input class="inp_panier1" style="margin:auto" type="submit" value="Mettre à jour"/><a style="display:inline-block" href="panier.php?delPanier=<?= $produit->id; ?>" class="del">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img id="action_l1" src="publicimgs/del.png"/></a><span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span></p>
				<p class="TTC">TTC : <?= number_format($produit->prix * 1.000 * $_SESSION['panier'][$produit->id],2,',',' ');if(isset($donnees210['ok']) AND $donnees210['ok']!='oui'){echo ' €';} else{echo ' $';} ?><span style="font-size:8px">(1)</span></p>
				<input type="hidden" id="valloop" value="<?php echo $k;?>"/>
				<br>
				<p id="line"> </p>
</div>
<?php endforeach; ?>
...
<div class="rowtotal">
	...
	<p>Grand Total<span style="font-size:8px">(1)</span>: <span style="color:#11DE48"><?php echo number_format($grand_total,2,',',' ');if(isset($donnees210['ok']) AND $donnees210['ok']!='oui'){echo ' €';} else{echo ' $';} ?></span></p></br>
	<br>
...
<div id="contenair_adr">
	...
	<div class="inp_subm_form_adresses">
		<a class="a_retour" href="tarifs.php">Retour</a>
...
require 'head.php';
	...
	<h2 class="h2_panier_bs" style="color:blue;margin:auto;text-align:center">Votre panier est vide</h2>
	<br><br>
	<p style="margin:auto;text-align:center"><a class="a_panier_bs" style="margin:auto;text-align:center;text-decoration:none;border-radius:10px/10px;color:white;background-color:black;padding:4px;" href="tarifs.php">Retour</a></p>
<?php }
?>
