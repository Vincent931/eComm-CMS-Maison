...
<?php 
$ref="1";
   $req2 = $bdd->query('SELECT reference, etat, acheteur, DATE_FORMAT(date_com, \'%d/%m/%Y\') AS date_com FROM commande');
...
if(isset($ref)AND !empty($ref)){
   if($ref!=$donnees2['reference']){
      $ref=$donnees2['reference'];?>
         <table>
            <tr>
               <td><?php if($donnees2['etat']=='oui'){echo '<span style="border:1px solid gray;color:blue">'.'Vente Validée'.'</span>';}else{echo '<span style="border:1px solid gray">'.'vente Non Validée'.'</span>';}?> du <?php echo $donnees2['date_com'];?> <a style="text-decoration:underline" href="vente-reussie.php?mail=<?php echo htmlspecialchars(urlencode($donnees2['acheteur']));?>&ref=<?php echo htmlspecialchars(urlencode($donnees2['reference'])) ?>" target="_blank">N° Référence : <?php echo htmlspecialchars($donnees2['reference']); ?></a></td>
            </tr>
            <tr>
               <td>
            </tr>
