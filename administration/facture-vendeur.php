...
$adresseVendeur.=' ';
if(isset($ste['adresse2']) AND !empty($ste['adresse2'])){ $adresseVendeur.=$ste['adresse'];$adresseVendeur.=' ';}
...
// entete des produits
$pdf->productHeaderAddRow('Produit', 55, 'L');
$pdf->productHeaderAddRow('Réf-obj', 20, 'C');
$pdf->productHeaderAddRow('Réf', 25, 'C');
...
// produit
$pdf->productAdd(array('Produit', 'Réf-Obj', 'Réf', 'prix U.TTC', 'Qté', 'Sous-total', 'Date'));
...
if(isset($_GET['ref']) AND isset($_GET['mail']) AND !empty($_GET['ref']) AND !empty($_GET['mail'])){
    ...
    $req= $bdd->prepare('SELECT devise, ref_obj, titre, quantite, prix, TVA, sous_total, total, livraison, grand_total, DATE_FORMAT(date_com, \'%d/%m/%Y\') AS date_com FROM commande WHERE reference=? AND acheteur=?');
...
$ref_obj=$donnees['ref_obj'];

if($donnees['devise']!='USD'){$pdf->productAdd(array(htmlspecialchars($donnees['titre']), $ref_obj, $ref, $prix.' Eur', htmlspecialchars($donnees['quantite']), $sous_total.' Eur', htmlspecialchars($donnees['date_com'])));}else{$pdf->productAdd(array(htmlspecialchars($donnees['titre']), $ref_obj, $ref, $prix.' Eur', htmlspecialchars($donnees['quantite']), $sous_total.' USD', htmlspecialchars($donnees['date_com'])));}
