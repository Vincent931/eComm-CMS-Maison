<div id="downsbank">
  <form name="DOWN" id="DOWN">
    <?php
$t=0; $req47=$bdd->query('SELECT * FROM upload');
While($donnees47=$req47->fetch())
{
  $t++;  
    ?><p><img style="width:100px;border:2px solid blue" src="../download/file.png"/></p>
    <p>Nom du téléchargement : <?php echo $donnees47['nom'];?></p>
    <input type="radio" name="down" id="down<?php echo $t;?>" value="<?php echo $donnees47['nom'];?>"/>
  </br>
  <?php
}
?>
...
AddText3(VALEUR10,image_name0,VALEUR30);
}

function ok3()
{ for (var a=1; a<=t; a++){
    if (document.getElementById('down'+a).checked==1) {
    image_name0 = document.getElementById('down'+a).value;
    }
  }
}

function AddText3(startTag,defaultText,endTag)
  {
    with(document.ch_form)
   {
      if (cle_down.createTextRange)
      {
         var text;
         cle_down.focus(cle_down.caretPos);
         cle_down.caretPos = document.selection.createRange().duplicate();
         if(cle_down.caretPos.text.length>0)
         {
            //gère les espaces de fin de sélection. Un double-click sélectionne le mot
            //+ un espace qu'on ne souhaite pas forcément...
            var sel = cle_down.caretPos.text;
            var fin = '';
            while(sel.substring(sel.length-1, sel.length)==' ')
            {
               sel = sel.substring(0, sel.length-1)
            }
            cle_down.caretPos.text = startTag + sel;// + endTag;
         }
         else
            cle_down.caretPos.text = startTag+defaultText+endTag;
      }
      //MOZILLA/NETSCAPE support
      else if (cle_down.selectionStart || cle_down.selectionStart == "0")
      {
            var startPos = cle_down.selectionStart; //position du curseur au début de la sélection
            var endPos = cle_down.selectionEnd-1; //position du curseur en fin de sélection
            var chaine = cle_down.value;
            ...
            {
                //startPos==endPos
                cle_down.value = startChaine + startTag + middleChaine + endTag + endChaine;
            }
            else
            {
                cle_down.value = startChaine + defaultText + endChaine;
            }
            //cle_down.selectionStart = startPos + defaultText.length;
            //cle_down.selectionEnd = endPos + defaultText.length;
            cle_down.focus();
      }
      else cle_down.value += startTag+defaultText+endTag;
    }
  }
</script>
</div>
</form>
<input type="button" onclick="initElement2()" name="yes" id="yes" value="Ajouter Téléchargement"/>
<div id="imgsbank">
...
//ne pas afficher bloc telechargement
$( "#downsbank" ).css( 'display', 'none' );
//voir bloc telechargement
$("#voir3").click(function() {
   $( "#downsbank" ).css( 'display', 'block' );
});
//cacher bloc image
