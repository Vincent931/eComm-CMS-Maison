...
//   
require('fpdf.php');
class facturePDFA extends FPDF{
// contenu
private $elementLst;
// logo
private $logoUrl;
...
function facturePDF($atextAdr1='', $atextAdr2='', $aFooter=''){
	...
	$this->SetCreator('www-1-zéro-Boutiques en ligne');
	$this->SetAuthor('VNPH');
	$this->AliasNbPages();
	$this->template = array();
...
//Titre Ajoute par MOI-MEME
public function setTitre($tUrl='', $tWidth=45, $tX=75, $tY=6){
	$this->titreUrl = $tUrl;
	$this->titrePosX =$tX;
	$this->titrePosY = $tY;
	$this->titreWidth = $tWidth;
}
// - - - - - - - - - - - - - - - - - - - - - - - - -
...
function Header(){
	...
	//AJOUTE PAR MOI-MEME
	if(!empty($this->titreUrl)){
		$this->Image($this->titreUrl, $this->titrePosX, $this->titrePosY, $this->titreWidth);
		$this->Ln(0);
	}
	// elements d'entete
