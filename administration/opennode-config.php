...
$req2=$bdd->query('SELECT * FROM opennode');
$yes=$req2->rowcount();
...
//effacement ancien dossier si changement
	...
	$req=$bdd->prepare('UPDATE opennode SET exist=:exist, passW=:passW, pairing_code=:pairing_code, label=:label, nom_doss=:nom_doss');
...
$req=$bdd->prepare('INSERT INTO opennode(exist, pairing_code, label, nom_doss) VALUES(:exist, :pairing_code, :label, :nom_doss)');
$req->execute(array('exist'=>$exist, 'passW'=>$passW, 'pairing_code'=>$pairing_code, 'label'=>$label, 'nom_doss'=>$nom_doss));
//creation du dossier
if (!is_dir('../'.$nom_doss)) {
    mkdir('../'.$nom_doss);
...
header("Location:opennode-ch.php");