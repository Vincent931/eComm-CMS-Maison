<?php session_start();
require 'texte1.php';
$req26=$bdd1->query('SELECT * FROM cheque');
$ste=$req26->fetch();

if(isset($_SESSION['reference']) AND isset($_SESSION['mail'])){
...
require 'texte1.php';
$req30=$bdd1->query('SELECT * FROM cheque');
$ste=$req30->fetch();
$adresseVendeur=$ste['prenom'];
$adresseVendeur.=' - ';
...
if(isset($_SESSION['reference']) AND isset($_SESSION['mail']) AND !empty($_SESSION['reference']) AND !empty($_SESSION['mail'])){

    $ref=$_SESSION['reference'];
    $acheteur=$_SESSION['mail'];
...
} else {echo '<h1 style="color:red">'.'GRAVE ERREUR'.'</h1>';}